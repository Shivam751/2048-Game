...
class Game(tk.Frame):
    def __init__(self):
        ...
        #Added functions for response on keys input 
    ...
    def start_game(self):
        ...
        self.matrix = [[0] * 4 for _ in range(4)]   #4*4 0 matrix
        ...
        self.cells[row][col]["number"].configure(
            ...
            text="2")
        while(self.matrix[row][col] != 0):  #for ensuring that same cell is not selected twice
            row = random.randint(0, 3)
            col = random.randint(0, 3)
        self.matrix[row][col] = 2
        self.cells[row][col]["frame"].configure(bg=c.CELL_COLORS[2])
    ...
    #shift all non empty cells to left edge
    def stack(self):
